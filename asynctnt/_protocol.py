...
from tarantool.request import RequestCall, RequestAuthenticate, \
    RequestInsert, RequestSelect, RequestPing
...
class IProto:
    ...
    def select(self, space_no, index_no, key, offset, limit, iterator):
        r = RequestSelect(self, space_no, index_no, key,
...
class BaseProtocol:
    ...
    def __init__(self, host, port, opts,
                 connected_fut, on_connection_lost, loop, **kwargs):
        self._host = host
        self._port = port
        self._opts = opts
        self._on_connection_lost = on_connection_lost
        self._loop = loop
        self._request_timeout = kwargs.get('request_timeout', 3)
    ...
    def _do_auth(self, username, password):
        ...
        def on_authorized(f):
            if f.cancelled():
                self._connected_fut.set_exception(
            ...
            if not e:
                ...
                logger.error('Tarantool[{}:{}] Authorization failed'.format(
                    self._host, self._port))
    ...
    def connection_made(self, transport):
        ...
        if sock is not None and \
                (not hasattr(socket, 'AF_UNIX') or sock.family != socket.AF_UNIX):  # nopep8
        ...
        for sync, fut in self._reqs.items():
            if fut and not fut.cancelled() and not fut.done():
                if exc is None:
                    ...
                    # fut.set_exception(ConnectionLostError('Connection to Tarantool lost'))  # nopep8
    ...
    def _execute(self, sync, request_data, *, timeout=0):
        ...
        if timeout and timeout > 0:
            # Client should wait the special timeout-ed future
            # (wrapping waiter)
            fut = asyncio.ensure_future(
    ...
    def call(self, func_name, *args, **kwargs):
        assert isinstance(func_name, str), \
            'Func name must be a str, got: {}'.format(type(func_name))
    ...
    def select(self, space_name, key=None, **kwargs):
        ...
        if isinstance(index_name, str):
            ...
            if idx is None:
                raise Exception(
                    'Index {} for space {} not found'.format(
                        index_name, space_name)
                )
            index_name = idx.iid

        return self._execute(*self._iproto.select(space_name,
                                                  index_name,
                                                  key,
                                                  offset,
                                                  limit,
