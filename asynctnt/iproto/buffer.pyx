...
cdef class WriteBuffer:
    ...
    cdef char *mp_encode_key_sequence(self, char *p, t,
        ...
        if isinstance(t, list) or t is None:
            return self.mp_encode_list(p, <list> t)
        elif isinstance(t, tuple):
            return self.mp_encode_tuple(p, <tuple> t)
        elif isinstance(t, dict) and fields is not None:
            return self.mp_encode_list(
    ...
    cdef char *_encode_update_ops(self, char *p, list operations,
        ...
        p = self.mp_encode_array(p, ops_len)
        if ops_len == 0:
            return p
        ...
        for operation in operations:
            ...
            if op == tarantool.IPROTO_OP_ADD \
                ...
                self._length += (p - begin)
                p = self.mp_encode_obj(p, op_argument)
    ...
    cdef int encode_request_call(self, str func_name, args) except -1:
        ...
        self._length += (p - begin)
        p = self.mp_encode_key_sequence(p, args)
    ...
    cdef int encode_request_select(self, SchemaSpace space, SchemaIndex index,
        ...
        self._length += (p - begin)
        p = self.mp_encode_key_sequence(p, key, index.fields, False)
    ...
    cdef int encode_request_insert(self, SchemaSpace space, t) except -1:
        ...
        p = mp_encode_uint(p, tarantool.IPROTO_TUPLE)
        self._length += (p - begin)
        p = self.mp_encode_key_sequence(p, t, space.fields, True)
    ...
    cdef int encode_request_update(self, SchemaSpace space, SchemaIndex index,
        ...
        p = self.mp_encode_uint(p, key_of_tuple)
        p = self.mp_encode_key_sequence(p, key_tuple, fields,
                                        default_fields_none)

        p = self.mp_encode_uint(p, key_of_operations)
        p = self._encode_update_ops(p, operations, space)

    cdef int encode_request_upsert(self, SchemaSpace space, t,
                                   list operations) except -1:
