...
class UpdateTestCase(BaseTarantoolTestCase):
    ...
    async def test__update_one_assign(self):
        ...
        res = await self.conn.update(self.TESTER_SPACE_ID, [1], [["=", 2, 2]])
    ...
    async def test__update_one_delete(self):
        ...
        res = await self.conn.update(self.TESTER_SPACE_ID, [2], [["#", 5, 1]])
        data[2].pop(5)
    ...
    async def test__update_one_minus(self):
        ...
        res = await self.conn.update(self.TESTER_SPACE_ID, [0], [["-", 2, 1]])
        data[0][2] -= 1
    ...
    async def test__update_one_band(self):
        ...
        res = await self.conn.update(self.TESTER_SPACE_ID, [1], [["&", 2, 1]])
        data[1][2] &= 1
        self.assertResponseEqual(res, [data[1]], "Body ok")
    ...
    async def test__update_splice_bytes(self):
        ...
        res = await self.conn.update(
            self.TESTER_SPACE_ID, [1], [[b":", 1, 1, 3, "!!!"]]
        )
    ...
    async def test__update_splice_wrong_args(self):
        ...
        with self.assertRaisesRegex(
            TypeError, r"Operation field_no must be " r"of either int or str type"
        ):
            await self.conn.update(
                self.TESTER_SPACE_ID, [1], [[":", {}, {}, {}, ":::"]]
            )

        with self.assertRaisesRegex(TypeError, r"Unknown update operation type `yo`"):
            await self.conn.update(self.TESTER_SPACE_ID, [1], [["yo", 1, 2, 3]])
    ...
    async def test__update_multiple_operations(self):
        ...
        res = await self.conn.update(self.TESTER_SPACE_ID, [1], operations)
        self.assertResponseEqual(res, [t], "Body ok")

    async def test__update_by_name(self):
        data = await self._fill_data()

        res = await self.conn.update(self.TESTER_SPACE_NAME, [1], [["=", 2, 2]])
    ...
    async def test__update_by_name_no_schema(self):
        ...
        with self.assertRaises(TarantoolSchemaError):
            await self.conn.update(self.TESTER_SPACE_NAME, [1], [["=", 2, 2]])
    ...
    async def test__update_by_index_id(self):
        ...
        try:
            ...
            res = await self.conn.update(
                self.TESTER_SPACE_ID, [data[0][2]], [("=", 2, 1)], index=index_id
            )
    ...
    async def test__select_by_index_name(self):
        ...
        try:
            ...
            res = await self.conn.update(
                self.TESTER_SPACE_ID, [data[0][2]], [["=", 2, 1]], index=index_name
            )
    ...
    async def test__update_by_index_id_no_schema(self):
        ...
        try:
            await self.conn.update(self.TESTER_SPACE_ID, [0], [["=", 2, 1]], index=0)
    ...
    async def test__update_by_index_name_no_schema(self):
        ...
        with self.assertRaises(TarantoolSchemaError):
            await self.conn.update(
                self.TESTER_SPACE_NAME, [0], [["=", 2, 1]], index="primary"
            )

    async def test__update_operations_none(self):
        data = await self._fill_data()
        try:
            res = await self.conn.update(self.TESTER_SPACE_NAME, [data[0][0]], None)
    ...
    async def test__update_dict_key(self):
        ...
        res = await self.conn.update(self.TESTER_SPACE_ID, {"f1": 0}, [["+", 3, 1]])
        data[0][3] += 1
        self.assertResponseEqual(res, [data[0]], "Body ok")
